...
Then(
  ...
  function (this: RoleXWorld, text: string) {
...
Then(
  "persona of {string} should be tagged @retired",
...
Then(
  "goal {string} should be marked @done",
  function (this: RoleXWorld, name: string) {
    const roleKey = this.individualSystem.ctx?.structure;
...
Then(
  "goal {string} should be marked @abandoned",
  function (this: RoleXWorld, name: string) {
    const roleKey = this.individualSystem.ctx?.structure;
...
Then(
  "focused plan should be {string}",
  function (this: RoleXWorld, plan: string) {
    const roleKey = this.individualSystem.ctx?.structure;
    const focus = this.graph.getNode(roleKey!)?.state?.focus;
    const goalNode = this.graph.getNode(focus!);
    const focusPlan = goalNode?.state?.focusPlan;
    assert.ok(focusPlan?.endsWith(`/${plan}`), `Focused plan should be "${plan}" but is "${focusPlan}"`);
  },
);
...
Then(
  "task {string} should be marked @done",
  function (this: RoleXWorld, name: string) {
    const roleKey = this.individualSystem.ctx?.structure;
    const content = this.platform.readContent(`${roleKey}/${name}`);
...
Then(
  "conclusion {string} should exist",
  function (this: RoleXWorld, name: string) {
    const roleKey = this.individualSystem.ctx?.structure;
    assert.ok(
      this.graph.hasNode(`${roleKey}/${name}-conclusion`),
      `Conclusion "${name}" not found`,
    );
  },
);

Then(
  /^experience\.insight "([^"]*)" should exist$/,
  function (this: RoleXWorld, name: string) {
    const roleKey = this.individualSystem.ctx?.structure;
    ...
    assert.ok(!this.graph.getNode(`${roleKey}/${name}`)?.shadow, `Insight "${name}" should not be shadowed`);
  },
);
...
Then(
  /^experience\.insight "([^"]*)" should not exist$/,
  function (this: RoleXWorld, name: string) {
    const roleKey = this.individualSystem.ctx?.structure;
    const node = this.graph.getNode(`${roleKey}/${name}`);
...
Then(
  /^knowledge\.pattern "([^"]*)" should exist$/,
  function (this: RoleXWorld, name: string) {
    const roleKey = this.individualSystem.ctx?.structure;
...
Then(
  /^knowledge\.pattern "([^"]*)" should not exist$/,
  function (this: RoleXWorld, name: string) {
    const roleKey = this.individualSystem.ctx?.structure;
    const node = this.graph.getNode(`${roleKey}/${name}`);
...
Then(
  ...
  function (this: RoleXWorld, name: string) {
    const roleKey = this.individualSystem.ctx?.structure;
    assert.ok(this.graph.hasNode(`${roleKey}/${name}`), `Pattern "${name}" not found`);
    assert.ok(!this.graph.getNode(`${roleKey}/${name}`)?.shadow, `Pattern "${name}" should not be shadowed`);
  },
);

Then(
  /^knowledge\.theory "([^"]*)" should exist$/,
  function (this: RoleXWorld, name: string) {
    const roleKey = this.individualSystem.ctx?.structure;
...
Then(
  ...
  function (this: RoleXWorld, role: string, org: string) {
    assert.ok(
      this.graph.hasEdge(org, role),
      `"${role}" should be a member of "${org}"`,
    );
  },
);
...
Then(
  ...
  function (this: RoleXWorld, role: string, org: string) {
    assert.ok(
      !this.graph.hasEdge(org, role),
      `"${role}" should NOT be a member of "${org}"`,
    );
  },
);
...
Then(
  ...
  function (this: RoleXWorld, role: string, position: string) {
    assert.ok(
      ...
      `"${role}" should be assigned to "${position}"`,
    );
  },
);
